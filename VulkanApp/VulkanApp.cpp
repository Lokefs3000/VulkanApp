...
void initWindow() {
	...
	window = SDL_CreateWindow("Vulkan Window", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, WIDTH, HEIGHT, SDL_WINDOW_VULKAN | SDL_WINDOW_INPUT_GRABBED);
...
void createInstance() {
	...
	if (result != VK_SUCCESS) {
		int err = (int)result;
		throw std::runtime_error(("failed to create instance " + std::to_string(err)).c_str());
	}
}
...
void setupDebugMessenger() {
	...
	if (result != VK_SUCCESS) {
		throw std::runtime_error(("failed to setup debug messenger " + std::to_string(result)).c_str());
	}
}
...
void createLogicalDevice() {
	...
	if (result != VK_SUCCESS) {
		throw std::runtime_error(("failed to create logical device " + std::to_string(result)).c_str());
	}
...
void createFramebuffers() {
	...
	for (size_t i = 0; i < swapChainImageViews.size(); i++) {
		...
		if (result != VK_SUCCESS) {
			throw std::runtime_error("failed to create framebuffer " + std::to_string(result));
		}
	}
}
...
void mainLoop() {
	while (true)
	{
		...
		if (SDL_PollEvent(&sdlEvent)) {
			...
			//if (sdlEvent.window.event == SDL_WINDOWEVENT_RESIZED) framebufferResized = true;
		}
...
int main(int argc, char* argv[])
{
	try
	{
		system("sh C:/Users/Loke/source/repos/DirectXEngine/VulkanApp/VulkanApp/shaders/compile.bat");
